...
public final class CandidateSpecification {
	 ...
	 private CandidateSpecification() {
	    ...
	    public static Specification<Candidate> withFilter(CandidateFilter filter) {

	        return (root, query, cb) -> {

	            var predicate = cb.conjunction();

	            if (filter.getCandidateId() != null && !filter.getCandidateId().isBlank()) {
	            ...
	            if (filter.getPhone() != null && !filter.getPhone().isBlank()) {
	                predicate = cb.and(predicate,
	                        cb.equal(root.get("phone"), filter.getPhone()));
	            }
	            ...
	            if (filter.getCandidateStatus() != null) {
	                predicate = cb.and(predicate,
	                        cb.equal(root.get("candidateStatus"),
	                                filter.getCandidateStatus()));
	            }

	            if (filter.getExperience() != null) {
	                predicate = cb.and(predicate,
	                        cb.greaterThanOrEqualTo(root.get("experience"),
